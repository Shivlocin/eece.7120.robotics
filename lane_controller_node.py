...
class lane_controller(object):
    ...
    def updatePose(self, pose_msg):
        ...
        # Student implementation for distance error PID.
        ###### OLD CONTROL #####
        ...
        # Create the deque objects which will maintain our history for the integral portion of our PID controller.
        omega_history = deque(maxlen=15)
        delta_history = deque(maxlen=15)

        # Sanity check. If the queue container is full, remove the last element and push the new value.
        # Take the current error and multiply it by delta_t (time that has passed) which gives us the area of the error
        # under the curve representing the graph of error values over time (integration)
        if len(omega_history) != 0 and len(omega_history) < 14:
            last_omega = omega_history.popleft()
            omega_history.append(self.heading_err * self.delta_t)
        else:
            last_omega = self.heading_err * self.delta_t
            omega_history.append(self.heading_err * self.delta_t)

        #
        # Do the same for distance history
        #
        if len(delta_history) != 0 and len(delta_history) < 14:
            last_delta = delta_history.popleft()
            delta_history.append(self.cross_track_err * self.delta_t)
        else:
            last_delta = self.cross_track_err * self.delta_t
            delta_history.append(self.cross_track_err * self.delta_t)

        # Get the sum total of the error "area" under the error curve i.e. the integral.
        omega_error = 0
        for elem in omega_history:
            omega_error += elem

        # Get the sum total of the error "area" under the delta_error curve i.e. the integral.
        delta_error = 0
        for elem in delta_history:
            delta_error += elem

        # Calculate the error rates for the derivative portion.
        omega_error_rate = (self.heading_err - last_omega) / self.delta_t
        delta_error_rate = (self.cross_track_err - last_delta ) / self.delta_t

        # Use the PID controller discussed in class to generate adjusted omega and delta values. Feed these values back
        # back into the original duckietown controller.
        omega = self.k_p * self.heading_err + self.k_i * omega_error + self.k_d * omega_error_rate
        delta = self.k_p * self.cross_track_err + self.k_i * delta_error + self.k_d * delta_error_rate

        #
        # Put the above into their controller just for funsiez
        #
        omega = self.k_d * (0.22/self.v_bar) * delta + self.k_theta * (0.22/self.v_bar) * omega
        omega += omega_feedforward
